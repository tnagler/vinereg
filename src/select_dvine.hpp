...
public:
    ...
    , controls_(controls)
    {
        ...
        hfunc1_[0] = data_.col(0);
        if (var_types_[0] == "d") {
            hfunc1_sub_[0] = data_.col(p_ + 1);
        }
    }
    ...
    void extend_fit(size_t var)
    {
        ...
        for (size_t t = 0; t < selected_vars_.size() + 1; t++) {
            ...
            if ((var_types[0] == "d") | (var_types[1] == "d")) {
                ...
                u_e.col(2) = (var_types[0] == "d") ? hfunc1_sub_[t] : u_e.col(0);
                u_e.col(3) = (var_types[1] == "d") ? hfunc2_sub_[t] : u_e.col(1);
            }
            ...
            hfunc1_[t] = pcs_[t].hfunc1(u_e);
            if (var_types[1] == "d") {
                auto u_e_sub = u_e;
                u_e_sub.col(1) = u_e.col(3);
                hfunc1_sub_[t] = pcs_[t].hfunc1(u_e_sub);
            }

            hfunc2_[t + 1] = pcs_[t].hfunc2(u_e);
            if (var_types[0] == "d") {
                auto u_e_sub = u_e;
                u_e_sub.col(0) = u_e.col(2);
                hfunc2_sub_[t + 1] = pcs_[t].hfunc2(u_e_sub);
            }
        }

        // shift hfunc1 entries and fill first tree with observations
        std::rotate(hfunc1_.begin(), hfunc1_.end() - 1, hfunc1_.end());
