...
#### Fit model
```{r, cache=TRUE}
npdvqr_fit <- vinereg(count ~ ., data = bikedata, family_set = "nonpar")
...
```{r me_temperature, cache=TRUE, fig.width=4, fig.height=4}
plot_marginal_effects(
    covs = select(bikedata, temperature), 
    preds = npdvqr_pred
)
```
```{r me_humidity, cache=TRUE, fig.width=4, fig.height=4, message=FALSE}
plot_marginal_effects(covs = select(bikedata, humidity), preds = npdvqr_pred) +
    xlim(c(25, 100))
```
```{r me_windspeed, cache=TRUE, fig.width=4, fig.height=4, message=FALSE}
plot_marginal_effects(covs = select(bikedata, windspeed), preds = npdvqr_pred) 
```
```{r me_month, cache=TRUE, fig.width=4, fig.height=4, message=FALSE}
...
```
```{r me_weathersituation, cache=TRUE, fig.width=4, fig.height=4, message=FALSE}
plot_marginal_effects(covs = select(bikedata, weathersituation), 
                      preds = npdvqr_pred) +
    scale_x_discrete(limits = 1:3,labels = c("good", "medium", "bad"))
```

```{r me_weekday, cache=TRUE, fig.width=4, fig.height=4, message=FALSE}
...
```
```{r me_workingday, cache=TRUE, fig.width=4, fig.height=4, message=FALSE}
plot_marginal_effects(covs = select(bikedata, workingday), preds = npdvqr_pred) +
    scale_x_discrete(limits = 0:1, labels = c("no", "yes")) +
    geom_smooth(method = "lm", se = FALSE) +
    xlim(c(0, 1))
```
```{r me_season, cache=TRUE, fig.width=4, fig.height=4, message=FALSE}
